...
**Every time** you call `web_fetch` or get useful results from `web_search`.
Do it in the same turn as your response — don't plan to "save it later".
...
Focus on **primary sources**. If a page cites a source, fetch the source too.
If a primary source is inaccessible, try the WayBack machine:
