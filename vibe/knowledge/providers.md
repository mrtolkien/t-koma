...
3. Export the provider in `t-koma-gateway/src/models/mod.rs`.
4. Initialize the provider in `t-koma-gateway/src/main.rs`.
5. Add provider type in `t-koma-core/src/config.rs`.
6. Add provider variant in `t-koma-core/src/message.rs` if needed.
