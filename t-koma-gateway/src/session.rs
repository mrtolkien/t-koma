...
use t_koma_db::{ContentBlock as DbContentBlock, DbPool, MessageRole, SessionRepository};
...
impl SessionChat {
    ...
    pub async fn chat(&self, session_id: &str, user_id: &str, message: &str) -> Result<String, ChatError> {
        ...
        info!("[session:{}] Chat message from user {}", session_id, user_id);
    ...
    async fn save_tool_results(&self, session_id: &str, tool_results: &[DbContentBlock]) {
        if let Err(e) = SessionRepository::add_message(
            self.db.pool(),
            session_id,
            MessageRole::User,
