            ...
            Message::Text(text) => {
                match serde_json::from_str::<WsMessage>(&text) {
                    ...
                    Ok(WsMessage::Chat { session_id, content }) => {
                        ...
                        // Get the selected model entry
                        let selected_entry = state
                            .get_model_by_alias(&selected_model_alias)
                            .unwrap_or_else(|| state.default_model());
                        let provider = selected_entry.client.as_ref();
                        ...
                        // Send to provider with conversation history and tools
                        match provider.send_conversation(
                            Some(system_blocks),
                            api_messages,
                            tools,
                            Some(&content),
